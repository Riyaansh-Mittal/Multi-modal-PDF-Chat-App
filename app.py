...
import re
import fitz  # PyMuPDF
import shutil  # To clear the images directory
# import torch
# from PIL import Image
# from transformers import CLIPProcessor, CLIPModel
# from transformers import BlipProcessor, BlipForConditionalGeneration
...
# Define the directory for storing extracted images
...
Question:
{question}??? Analyze the document thoroughly, including all forms of images, to provide a detailed response. If relevant data is found in images such as charts or graphs, reference them clearly and avoid contradicting earlier statements. For general questions, offer a relevant and informative answer based on your internal knowledge and the document content if it enhances the response. AND never mention the document in response.
...
# Streamlit app logic
def main():
    # Clear the extracted images directory upon each page reload
    ...
    if user_query:
        complete_object = create_complete_object(text, user_query)
        # Load image documents from the freshly populated local directory
        image_documents = SimpleDirectoryReader(images_dir).load_data()
        # Use Gemini model to complete the query based on the context
        response = gemini_model.complete(
            prompt=complete_object,
            image_documents=image_documents  # Pass image paths directly
